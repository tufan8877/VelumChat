...
class ProfileProtection {
  ...
  storeProfile(profile: UserProfile): void {
    ...
    // Store minimal profile metadata in cookie as last resort (NO privateKey)
    // Store minimal profile metadata in cookie as last resort (NO privateKey)
    const safeProfile = { id: profile.id, username: profile.username, publicKey: profile.publicKey };
    document.cookie = `user_profile_${profile.username}=${btoa(JSON.stringify(safeProfile))}; path=/; expires=Fri, 31 Dec 2099 23:59:59 GMT`;
  ...
  retrieveProfile(username?: string): UserProfile | null {
    ...
    // Try cookie backup (metadata-only). If legacy cookie contains privateKey, migrate once and then overwrite cookie safely.
    if (username) {
      ...
      for (let cookie of cookies) {
        ...
        if (cookie.indexOf(cookieName) === 0) {
          try {
            ...
            const cookieProfile = JSON.parse(atob(profileData));
            console.log("✅ Found profile in cookie backup:", cookieProfile.username);

            // Legacy migration: if cookie had privateKey, restore and immediately rewrite cookie WITHOUT privateKey
            if (cookieProfile.privateKey) {
              localStorage.setItem("user", JSON.stringify(cookieProfile));
              sessionStorage.setItem("user_backup", JSON.stringify(cookieProfile));

              const safeProfile = { id: cookieProfile.id, username: cookieProfile.username, publicKey: cookieProfile.publicKey };
              document.cookie = `user_profile_${cookieProfile.username}=${btoa(JSON.stringify(safeProfile))}; path=/; expires=Fri, 31 Dec 2099 23:59:59 GMT`;

              console.log("🔐 Migrated legacy cookie (removed privateKey from cookie)");
              return cookieProfile;
            }

            // Metadata-only cookie can't recover privateKey; keep looking elsewhere (already tried local/session/memory above)
            console.log("🔐 Cookie contains metadata only (no privateKey); cannot recover privateKey from cookie.");
          } catch (error) {
